...
| [B-069] | 2026-02-20 | BUDGET | PROG | Bug Fix B-007: Zusatzleistungen im Angebot sichtbar machen |
| [B-070] | 2026-02-20 | BUDGET | PROG | B-008: Angebote speichern + Verlauf (DB + Frontend) |
| [B-071] | 2026-02-20 | BUDGET | PL | Refactoring: Budgetangebot.jsx in 8 Dateien aufgesplittet (Kontext-Optimierung) |
...
| [K-003] | 2026-02-20 | KATEG | TEST | Edge Functions Health-Check + Kategorisierungs-Qualitaet |

> **Aeltere Eintraege:** siehe [MASTER_LOG_ARCHIVE.md](MASTER_LOG_ARCHIVE.md) (R-001 bis R-037, Januar 2026)

---

## ═══ LOG START ═══

> Archivierte Detail-Eintraege (vor Feb 2026): siehe [MASTER_LOG_ARCHIVE.md](MASTER_LOG_ARCHIVE.md)
...
## [B-069] Programmierer: Bug Fix B-007 - Zusatzleistungen im Angebot
**Datum:** 2026-02-20
**Workflow:** BUDGET

### Kontext
Wenn Zusatzleistungen (Montage, Demontage, Entsorgung) als Checkboxen in Step 1 ausgewaehlt wurden, hat die budget-ki Edge Function die Kosten korrekt berechnet und im Response-Feld `data.montage` zurueckgegeben. Allerdings wurden diese Kosten im Frontend an zwei Stellen ignoriert:
1. **StepZusammenfassung (Step 3):** Die Netto-/Brutto-Summe wurde nur aus `editedPositions.gesamtpreis` berechnet, ohne Montage/Demontage/Entsorgung einzubeziehen.
2. **handleGenerateDocument (Step 4):** Gleicher Fehler bei der Netto-Berechnung, PLUS das `montage`-Objekt wurde nicht an die budget-dokument Edge Function gesendet. Dadurch fehlten die Zusatzleistungen komplett im generierten Angebot/PDF.

### Durchgefuehrt
**Datei:** `dashboard/src/pages/Budgetangebot.jsx`

1. **StepZusammenfassung (Step 3):**
   - Montage-Daten aus `result.data.montage` extrahiert
   - `montageKosten` (Summe aus montage + demontage + entsorgung) zum Netto addiert
   - Einzelne Zusatzleistungen (Montage, Demontage, Entsorgung) als separate Zeilen in der Preiszusammenfassung angezeigt
   - Fenster/Tueren-Zeile zeigt jetzt `positionenNetto` statt leeres `breakdown.fenster`

2. **handleGenerateDocument (Step 4):**
   - Montage-Kosten in die Netto-/MwSt-/Brutto-Berechnung einbezogen
   - `montage`-Objekt (mit montage/demontage/entsorgung Einzelwerten) an die budget-dokument Edge Function gesendet
   - Die Edge Function `budget-dokument` unterstuetzte das `montage`-Feld bereits (renderSummary mit montageRows), es wurde nur nie befuellt

### Ergebnis
- **Build:** OK (vite build erfolgreich, keine Fehler)
- **Vorher:** Zusatzleistungen in Checkboxen ausgewaehlt -> Budget-KI berechnet korrekt -> Frontend ignoriert Montage-Kosten in Summe und Dokument
- **Nachher:** Zusatzleistungen erscheinen als eigene Zeilen in der Preiszusammenfassung (Step 3), werden in Gesamtsumme eingerechnet, und werden an die Dokument-Generierung (Step 4) uebergeben wo sie als Montage/Demontage/Entsorgung im Angebot erscheinen

### Naechster Schritt
- Browser-Test durchfuehren (echte KI-Abfrage mit Montage-Checkboxen)
- Pruefen ob die Entsorgung-Option im budget-ki korrekt mit eigenem Flag statt `optionen.demontage` gesteuert wird (aktuell nutzt `enrichBudgetResult` in budget-ki den `demontage`-Flag fuer Entsorgung - funktioniert weil meist zusammen aktiviert, aber technisch nicht korrekt)

---

## [K-003] Tester: Edge Functions Health-Check + Kategorisierungs-Qualitaet
**Datum:** 2026-02-20
**Workflow:** KATEG

### Kontext
Regelmaessiger Health-Check der produktiven Edge Functions und Kategorisierung.
Pruefung aller 3 geschuetzten Edge Functions, pg_cron Jobs und Email-Subscriptions.

### Durchgefuehrt
1. **Edge Function Logs** abgerufen (service: edge-function, postgres, auth)
2. **SQL-Queries** auf `documents`-Tabelle (Verarbeitungs-Status, Kategorien, Stuck/Error, Emails)
3. **pg_cron Job-Status** geprueft (3 Jobs aktiv)
4. **Email-Subscriptions** geprueft (4 aktive Subscriptions, 2 Postfaecher)

### Ergebnis

**Edge Functions:**
- `process-document` v34 (Deploy 52): OK - Letzte Aktivitaet 20.02. ~11:16 UTC, alle HTTP 200
- `email-webhook` v28: OK - Alle HTTP 202, Execution Time 86-907ms
- `batch-process-pending` v3: OK - Alle 15 Min, alle HTTP 200
- `process-email` v38: WARNUNG - Durchgehend HTTP 401, wird von email-webhook umgangen

**pg_cron Jobs:** Alle 3 aktiv, batch-process-pending 20/20 succeeded
**Email-Subscriptions:** 4 aktiv, 2 Postfaecher, Expiry 22.02., kein Fehler
**Kategorisierung 7 Tage:** 195 Docs, 100% kategorisiert, 0 Stuck/Error
**Top-Kategorien:** Email_Eingehend (175), Sonstiges_Dokument (15), Eingangslieferschein (2)
**Processing-Status Gesamt:** 2.564 Docs (1.189 done, 1.152 pending alt, 223 queued)

### Naechster Schritt
1. process-email 401: Kein Handlungsbedarf (email-webhook uebernimmt)
2. 1.152 pending Docs (Dez/Jan): Pruefen ob Nachverarbeitung gewuenscht
3. email_kategorie-Feld wird fuer neue Emails nicht befuellt - pruefen ob gewollt
4. Naechster Health-Check in 1-2 Wochen

---

## [B-070] Programmierer: Angebote speichern + Verlauf (B-008)

**Datum:** 2026-02-20
**Workflow:** BUDGET
**Rolle:** PROG
**Auftrag:** B-008 - Angebote speichern + Verlauf im Dashboard

### Was wurde gemacht

**Phase 1: DB-Design + Migration (Supabase)**
- Tabelle `budgetangebote` erstellt: UUID PK, kontakt_id (FK), kontakt_name, projekt_bezeichnung, status (entwurf/versendet/angenommen/abgelehnt), profilsystem, montage/demontage/entsorgung Flags, netto/mwst/brutto Summen, montage_kosten JSONB, version, timestamps
- Tabelle `budgetangebot_positionen` erstellt: UUID PK, budgetangebot_id (FK CASCADE), pos_nr, raum, typ, bezeichnung, breite/hoehe_mm, menge, einzelpreis/gesamtpreis, details JSONB
- RLS aktiviert (Allow all for now)
- Indexes auf kontakt_id, status, erstellt_am, budgetangebot_id

**Phase 2: Speichern-Funktion (Budgetangebot.jsx)**
- `handleSave()`: Speichert/aktualisiert Angebot + Positionen in Supabase
- Insert bei neuem Angebot, Update bei bestehendem (savedAngebotId)
- Positionen: Delete + Re-Insert Strategie (einfach, zuverlaessig)
- URL wird nach Speichern auf `?id=xxx` aktualisiert
- Erfolgs-/Fehlermeldungen in Step 3 und Step 4

**Phase 3: Lade-Funktion (Budgetangebot.jsx)**
- `loadBudgetangebot(id)`: Laedt Angebot + Positionen aus Supabase
- Laedt optional Kontakt-Details (Adresse, Telefon, Email) nach
- Befuellt alle State-Variablen (Positionen, Kunde, Optionen, Montage)
- URL-Parameter `?id=xxx` wird beim Mount gelesen
- Springt automatisch zu Step 2 wenn Positionen vorhanden

**Phase 4: Verlaufs-Ansicht (BudgetangebotVerlauf.jsx)**
- Neue Seite `/budgetangebot-verlauf` mit Tabelle aller gespeicherten Angebote
- Spalten: Datum, Kunde, Projekt, Brutto/Netto, Status, Version, Aktionen
- Suche nach Kundenname/Projektbezeichnung
- Status-Filter (Entwurf/Versendet/Angenommen/Abgelehnt)
- Inline-Status-Aenderung per Dropdown
- Pagination (20 pro Seite)
- Loeschen mit Bestaetigung
- "Oeffnen" navigiert zu `/budgetangebot?id=xxx`

**Phase 5: Navigation (App.jsx)**
- Neuer Nav-Eintrag "Angebotsverlauf" mit History-Icon
- Route `/budgetangebot-verlauf` registriert
- "Alle Angebote"-Button im Budgetangebot-Header

### Geaenderte Dateien
- `dashboard/src/pages/Budgetangebot.jsx` - Save/Load + URL-Params + Header
- `dashboard/src/pages/BudgetangebotVerlauf.jsx` - NEU: Verlaufs-Ansicht
- `dashboard/src/App.jsx` - Route + Navigation ergaenzt

### Build-Status
- `npm run build`: ERFOLGREICH (2.89s, keine Warnings)

### Naechste Schritte
1. Browser-Test: Speichern, Laden, Verlauf durchklicken
2. E-Mail-Versand (Button-Placeholder vorhanden)
3. PDF-Speicherung an Budgetangebot verknuepfen

---

## [B-071] Projektleiter: Refactoring Budgetangebot.jsx → 8 Dateien
**Datum:** 2026-02-20
**Workflow:** BUDGET

### Kontext
Budgetangebot.jsx war mit 1.948 Zeilen (~25.000 Tokens) der groesste Kontextfresser.
Bei jedem Agenten-Aufruf musste die komplette Datei gelesen werden, auch wenn nur ein
kleiner Teil relevant war. Kontext-Analyse der parallelen Agenten (B-069, K-003, B-070)
zeigte: 2 von 3 Agenten luden die volle Datei → ~50.000 Tokens verschwendet.

### Durchgefuehrt
Aufsplitten in 8 Dateien unter `dashboard/src/pages/budgetangebot/`:

| Datei | Zeilen | Inhalt | Wann lesen |
|-------|--------|--------|------------|
| `constants.js` | ~75 | SYSTEME, MWST_SATZ, FIRMA_INFO, Helpers | Preise/Formatierung aendern |
| `ui.jsx` | ~130 | StepIndicator, ConfidenceBadge, EditableCell | UI-Komponenten aendern |
| `KundenSuche.jsx` | ~200 | searchKontakte(), KundenSuchModal | Kundensuche aendern |
| `StepEingabe.jsx` | ~250 | Step 1 Freitext + Optionen | Eingabe-Seite aendern |
| `StepPositionen.jsx` | ~180 | Step 2 Positionstabelle | Tabelle aendern |
| `StepZusammenfassung.jsx` | ~300 | Step 3 Preise + Montage | Preisanzeige aendern |
| `StepVorschau.jsx` | ~110 | Step 4 PDF-Vorschau | Vorschau aendern |
| `Budgetangebot.jsx` | ~480 | Orchestrator (State + Handlers) | API/State-Logik aendern |

### Ergebnis
- Build OK (3.26s, keine Fehler)
- Token-Einsparung: Agent muss nur noch relevante Datei lesen (~200-300 Zeilen statt 1.948)
- Geschaetzter Token-Vorteil: ~15.000-20.000 Tokens pro Agenten-Aufruf
- BACKLOG-Verweise aktualisiert (B-005, B-006, G-001)
- B-007 aus BACKLOG geloescht (erledigt durch B-069)

### Naechster Schritt
Workflow-CLAUDE.md ggf. um Datei-Map ergaenzen, damit Agenten wissen welche Datei sie lesen sollen.

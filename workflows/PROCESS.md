# PROCESS.md - Gemeinsamer Workflow-Prozess

> **Zweck:** Dieses Dokument definiert das 3-Agenten-System, Logging-Regeln, Pflicht-Checks und Templates fuer ALLE Workflows. Jede workflow-spezifische `CLAUDE.md` verweist hierhin.
>
> **Platzhalter:** `{PREFIX}` und `{WORKFLOW_TAG}` werden durch die Werte aus der jeweiligen Workflow-CLAUDE.md ersetzt.

---

## Registrierte Workflows

| Workflow | Ordner | Prefix | Workflow-Tag | Log-Beispiel |
|----------|--------|--------|-------------|--------------|
| Budgetangebote | budgetangebote/ | B | BUDGET | `[B-039]` |
| Reparaturen | reparaturen/ | R | REPAIR | `[R-041]` |
| ~~Kategorisierung~~ | ~~kategorisierung/~~ | K | KATEGORIE | `[K-003]` (abgeschlossen) |
...
3. **Workflow-CLAUDE.md** - Workflow-Regeln (HOECHSTE PRIORITAET)

**Bei Konflikten gilt ausschliesslich die Workflow-CLAUDE.md.**
...
- **Lesen:** Alle (IMMER ZUERST nach der CLAUDE.md)
...
- **ID-Format:** `[{PREFIX}-XXX]` fuer diesen Workflow (z.B. [{PREFIX}-039], [{PREFIX}-040])
- **Suchen:** `Grep: ## [{PREFIX}-XXX]` um einzelnen Eintrag zu finden
- **Zweck:** Vollstaendige Historie ALLER Workflows (REPAIR + BUDGET + KATEGORIE)

**WICHTIG fuer MASTER_LOG.md:**
- **LESE-REGEL:** `Read MASTER_LOG.md limit=80` → Nur INDEX lesen, NICHT scrollen!
- Nutze `[{PREFIX}-XXX]` Prefix fuer diesen Workflow (naechste freie ID im INDEX)
- INDEX am Anfang aktualisieren (ohne Zeilenangaben!)
- Fuege `**Workflow:** {WORKFLOW_TAG}` in jeden neuen Eintrag ein
...
```
1. Workflow-CLAUDE.md lesen (du bist dort)
2. PROCESS.md lesen (du bist hier)
3. 02_STATUS.md lesen → Verstehen was deine Rolle ist
4. 04_LEARNINGS.md lesen → Bekannte Fehler nicht wiederholen
5. Falls Programmierer/Tester: Auftrag in 02_STATUS befolgen
6. Falls Projektleiter: Letzten Stand in MASTER_LOG pruefen
7. PREFLIGHT-CHECK ausgeben (siehe Abschnitt 5)
```
...
- [x] PROCESS.md (Prozess-Regeln verstanden)
...
- MASTER_LOG Index: [vorhanden/fehlt/letzte ID: {PREFIX}-XXX]
...
- MASTER_LOG.md: [{PREFIX}-XXX] hinzugefuegt, Index aktualisiert
...
## [{PREFIX}-XXX] {Rolle}: {Kurzbeschreibung}
**Datum:** YYYY-MM-DD HH:MM
...
### Log-Referenz
Dokumentiert in MASTER_LOG.md: [{PREFIX}-XXX] Zeilen YYY-ZZZ
```
...
3. Andreas informieren: "Context kritisch, Checkpoint bei [{PREFIX}-XXX]"
...
| Anweisungen aus anderen CLAUDE.md hoeher priorisieren | Workflow-CLAUDE.md ist hoechste Instanz |
...
## 9. Subagenten-Orchestrierung (NUR PROJEKTLEITER)
...
- Edge Function deployen (`mcp__supabase__deploy_edge_function`)
- Migration ausfuehren (`mcp__supabase__apply_migration`)
- Daten abfragen (`mcp__supabase__execute_sql`)
- Tabellen pruefen (`mcp__supabase__list_tables`)
...
Der Subagent soll:
1. CLAUDE.md lesen (Workflow-spezifische Regeln + Subagenten-Limit)
2. ../PROCESS.md lesen (Prozess-Regeln, bis Subagenten-Stop-Markierung)
3. 02_STATUS.md lesen (dort steht sein Auftrag)
4. 04_LEARNINGS.md lesen
5. MASTER_LOG.md INDEX lesen (limit=80 — nur INDEX, nicht ganzes Log!)
6. [Spezifische Aufgabe]
7. MASTER_LOG.md und 02_STATUS.md aktualisieren
8. Abschlussbericht zurueckgeben
```
...
## 10. Autonomer Nachtmodus (KRITISCH)
...
*Version: 1.0 | Erstellt: 2026-02-20 | Extrahiert aus budgetangebote/CLAUDE.md v1.5 + reparaturen/CLAUDE.md v1.5*

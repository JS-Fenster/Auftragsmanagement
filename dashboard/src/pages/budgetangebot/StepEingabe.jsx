  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface-card rounded-lg shadow-sm border border-border-default">
        <button
          className="w-full flex items-center justify-between px-6 py-4 text-sm font-semibold text-text-primary hover:bg-surface-main transition-colors rounded-lg"
          ...
          <div className="px-6 pb-6 pt-0 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="relative">
                <label className="block text-xs text-text-secondary mb-1">Kundenname</label>
                <div className="flex gap-1">
                  <div className="relative flex-1">
                    <input
                      type="text"
                      className={`w-full border border-border-default rounded-lg py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand ${selectedKontaktId ? 'pl-3 pr-8 bg-green-50 border-green-300' : 'px-3'}`}
                    ...
                    {selectedKontaktId && (
                      <button
                        ...
                        className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded-full bg-surface-hover text-text-secondary hover:bg-red-100 hover:text-red-600 transition-colors"
                    ...
                    {kundenLoading && !selectedKontaktId && (
                      <Loader2 className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 animate-spin text-text-muted" />
                    )}

                    {/* Autocomplete Dropdown */}
                    {kundenVorschlaege.length > 0 && !selectedKontaktId && (
                      <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-surface-card border border-border-default rounded-lg shadow-lg max-h-60 overflow-y-auto">
                        {kundenVorschlaege.map(v => (
                          <button
                            key={v.kontakt_id}
                            className="w-full text-left px-3 py-2 hover:bg-brand-light transition-colors border-b border-gray-50 last:border-0"
                  ...
                  <button
                    type="button"
                    className="px-2.5 py-2 border border-border-default rounded-lg text-text-secondary hover:text-brand hover:border-blue-300 hover:bg-brand-light transition-colors shrink-0"
              ...
              <div>
                <label className="block text-xs text-text-secondary mb-1">E-Mail</label>
                <input
                  type="email"
                  className="w-full border border-border-default rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
      ...
      <div className="bg-surface-card rounded-lg shadow-sm border border-border-default p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* System Selection */}
          <div>
            <label className="block text-sm font-semibold text-text-primary mb-2">Profilsystem</label>
            <select
              className="w-full border border-border-default rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
          ...
          {/* Montage Checkboxes */}
          <div>
            <label className="block text-sm font-semibold text-text-primary mb-2">Zusatzleistungen</label>
      ...
      <div className="flex justify-center">
        <button
          className="inline-flex items-center gap-2.5 px-8 py-3.5 bg-btn-primary text-white text-base font-semibold rounded-xl hover:bg-btn-primary-hover disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg"

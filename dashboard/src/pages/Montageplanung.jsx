...
export default function Montageplanung() {
  ...
  return (
    <div className="min-h-screen bg-surface-main p-6" onClick={() => setSelectedEvent(null)}>
      {/* Header */}
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Calendar className="h-7 w-7 text-brand" />
          <h1 className="text-2xl font-bold text-text-primary">Montageplanung</h1>
        </div>
      </div>

      {/* Today Banner */}
      {todayEvents.length > 0 && (
        <div className="mb-4 rounded-lg bg-brand-light border border-blue-200 px-4 py-3 flex items-center gap-3">
      ...
      <div className="flex gap-4">
        {/* Calendar */}
        <div className="flex-1 min-w-0 rounded-lg bg-surface-card p-4 shadow-sm border border-border-default">
          {loading ? (
            <div className="flex h-96 items-center justify-center text-text-muted">Laden...</div>
          ) : (
            <FullCalendar
              ref={calendarRef}
        ...
        <div className="w-80 shrink-0">
          <div className="rounded-lg bg-surface-card shadow-sm border border-border-default">
            ...
            <div id="warteschlange-container" className="max-h-[calc(100vh-260px)] overflow-y-auto p-3 space-y-2">
              {warteschlange.length === 0 && (
                <p className="text-xs text-text-muted text-center py-4">Keine offenen Montagen</p>
              )}
              {warteschlange.map(p => (
                <div
                  key={p.id}
                  className="warteschlange-item rounded-lg border border-border-default bg-surface-main p-3 cursor-grab hover:shadow-md hover:border-blue-300 transition-all"
                  draggable="true"
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-text-primary truncate">{p.projekt_nummer}</p>
                      <p className="text-xs text-text-secondary truncate">{p.kundeName}</p>
                      {p.kontakte?.ort && <p className="text-xs text-text-muted">{p.kontakte.ort}</p>}
                    </div>
                  ...
                  {(p.auftrags_wert || p.angebots_wert) && (
                    <p className="mt-1 text-xs text-text-secondary">{formatCurrency(p.auftrags_wert || p.angebots_wert)}</p>
                  )}
                  ...
                  {inlineEdit?.projektId === p.id ? (
                    <div className="mt-2 space-y-1.5" onClick={e => e.stopPropagation()}>
                      <input
                        type="date"
                        className="w-full rounded border border-border-default px-2 py-1 text-xs"
                      ...
                      <div className="flex gap-1">
                        <button
                          ...
                          className="flex-1 rounded bg-btn-primary px-2 py-1 text-xs text-white hover:bg-btn-primary-hover disabled:opacity-40"
                        >
                          Speichern
                        </button>
                        <button
                    ...
                    <button
                      ...
                      className="mt-2 w-full rounded border border-dashed border-border-default py-1 text-[11px] text-text-secondary hover:border-blue-400 hover:text-brand transition-colors"
                    >
                      Termin setzen
                    </button>
                  )}
      ...
      {selectedEvent && (
        <div
          className="fixed z-50 w-80 rounded-lg bg-surface-card shadow-xl border border-border-default p-4"
          style={{ top: Math.min(popupPos.top, window.innerHeight - 280), left: popupPos.left }}
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-start justify-between mb-3">
            <h3 className="text-sm font-bold text-text-primary">{selectedEvent.projekt_nummer}</h3>
            <button onClick={() => setSelectedEvent(null)} className="text-text-muted hover:text-text-secondary">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-text-secondary">Kunde</span>
              <span className="font-medium text-text-primary">{getKundeName(selectedEvent.kontakte)}</span>
            </div>
            {selectedEvent.kontakte?.ort && (
              <div className="flex justify-between">
                <span className="text-text-secondary">Ort</span>
                <span className="text-text-primary">{selectedEvent.kontakte.ort}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-text-secondary">Status</span>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                selectedEvent.status === 'erledigt' ? 'bg-surface-hover text-text-secondary' :
                selectedEvent.status === 'montagebereit' ? 'bg-green-100 text-green-700' :
                'bg-amber-100 text-amber-700'
              }`}>{selectedEvent.status}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-secondary">Montage-Team</span>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-text-primary">{selectedEvent.montage_datum}</span>
            </div>
          ...
          <button
            ...
            className="mt-3 w-full rounded bg-btn-primary py-1.5 text-xs font-medium text-white hover:bg-btn-primary-hover"
          >
            Zum Projekt
          </button>
        </div>

...
import { searchKontakte, searchProjekte } from '../lib/search'
...
export default function CommandPalette() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // Zentrale Suche aus lib/search — Multi-Wort-Tokens, AND-Schnittmenge
      const [projekteData, kontakteData] = await Promise.all([
        searchProjekte(q, { limit: 5, fields: ['titel', 'projekt_nummer'] }),
        searchKontakte(q, { limit: 5, select: 'id, firma1, firma2, ort' }),
      ])

      setResults({
        actions: filteredActions,
        projekte: projekteData,
        kontakte: kontakteData,
      })
      setSelectedIndex(0)
      setLoading(false)
    }, 200)

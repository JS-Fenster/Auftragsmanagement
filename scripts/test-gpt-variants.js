/**
 * GPT-Varianten Vergleichstest für Aufmaßblatt-OCR
 *
 * Testet verschiedene GPT-Modelle und Reasoning-Stufen
 * mit echtem OCR-Text aus einem Aufmaßblatt.
 *
 * Usage: node scripts/test-gpt-variants.js
 *
 * Benötigt: OPENAI_API_KEY in .env oder Umgebungsvariable
 */
...
// Echter OCR-Text vom Aufmaßblatt (aus Supabase documents Tabelle)
...
// System-Prompt für Budgetangebot-Strukturierung
const SYSTEM_PROMPT = `Du bist ein Experte für Fenster-Aufmaßblätter bei J.S. Fenster & Türen.

AUFGABE: Extrahiere alle relevanten Informationen aus dem OCR-Text und korrigiere offensichtliche OCR-Fehler.

REGELN:
- Korrigiere Tippfehler kontextuell (z.B. "Dratex" → "DRUTEX", "Gosdlossen" → "Geschlossene")
- Interpretiere Zimmer-Kürzel (K6 → KG/Keller, 106 → OG/Obergeschoss)
- Maße sind in mm (auch wenn nicht explizit angegeben)
- Nutze "fertige_breite/hoehe" wenn vorhanden, sonst "innen_breite/hoehe"
- Zeilen ohne eigene Maße gehören zur vorherigen Position (Zusatzinfo)
...
// Test-Varianten
const VARIANTS = [
  { name: 'GPT-4o-mini', model: 'gpt-4o-mini', reasoning: null },
  { name: 'GPT-5.2 (none)', model: 'gpt-5.2', reasoning: 'none' },
  { name: 'GPT-5.2 (low)', model: 'gpt-5.2', reasoning: 'low' },
  { name: 'GPT-5.2 (medium)', model: 'gpt-5.2', reasoning: 'medium' },
  { name: 'GPT-5.2 (high)', model: 'gpt-5.2', reasoning: 'high' },
];
...
async function testVariant(variant) {
  ...
  const requestBody = {
    ...
    messages: [
      ...
      { role: 'user', content: `Analysiere dieses Aufmaßblatt:\n\n${OCR_TEXT}` }
    ],
    response_format: {
      type: 'json_schema',
      json_schema: {
  ...
  // Reasoning nur für GPT-5.x Modelle
...
function evaluateResult(result) {
  ...
  if (data.kunde?.name === 'Metschel Markus') {
    score += 10;
    details.push('✅ Kunde korrekt');
  } else {
    details.push(`❌ Kunde: "${data.kunde?.name}" (erwartet: "Metschel Markus")`);
  }

  // 2. Hersteller korrigiert (15 Punkte)
  if (data.profil?.hersteller?.toUpperCase() === 'DRUTEX') {
    score += 15;
    details.push('✅ Hersteller korrigiert (Dratex → DRUTEX)');
  } else {
    details.push(`❌ Hersteller: "${data.profil?.hersteller}" (erwartet: "DRUTEX")`);
  }

  // 3. System erkannt (10 Punkte)
  if (data.profil?.system?.includes('IDEAL') || data.profil?.system?.includes('NEO')) {
    score += 10;
    details.push('✅ System erkannt');
  } else {
    details.push(`❌ System: "${data.profil?.system}"`);
  }
  ...
  if (elementCount >= 3) {
    score += 15;
    details.push(`✅ ${elementCount} Elemente erkannt`);
  } else if (elementCount >= 2) {
    score += 8;
    details.push(`⚠️ Nur ${elementCount} Elemente (erwartet: 3+)`);
  } else {
    details.push(`❌ Nur ${elementCount} Elemente`);
  }

  // 5. Maße korrekt (20 Punkte)
  const expectedMeasures = [
    { b: 935, h: 2035 },   // Pos 1 Fertigmaß
    { b: 1001, h: 2116 },  // Pos 2 Innenmaß (kein Fertigmaß)
    { b: 1035, h: 1895 }   // Pos 5 Fertigmaß
  ];
  ...
  if (correctMeasures >= 3) {
    score += 20;
    details.push('✅ Alle Maße korrekt');
  } else if (correctMeasures >= 2) {
    score += 12;
    details.push(`⚠️ ${correctMeasures}/3 Maße korrekt`);
  } else {
    details.push(`❌ Nur ${correctMeasures}/3 Maße korrekt`);
  }
  ...
  if (hasKG || hasOG) {
    score += 10;
    details.push('✅ Zimmer interpretiert (K6→KG oder 106→OG)');
  } else {
    details.push('❌ Zimmer nicht interpretiert');
  }

  // 7. Montage-Checkboxen (10 Punkte)
  if (data.montage?.demontage && data.montage?.montage && data.montage?.entsorgung) {
    score += 10;
    details.push('✅ Montage-Checkboxen korrekt');
  } else {
    details.push('❌ Montage-Checkboxen fehlerhaft');
  }

  // 8. Korrekturen dokumentiert (10 Punkte)
  if (data.korrekturen?.length >= 2) {
    score += 10;
    details.push(`✅ ${data.korrekturen.length} Korrekturen dokumentiert`);
  } else {
    details.push(`⚠️ Nur ${data.korrekturen?.length || 0} Korrekturen`);
  }
...
async function runTests() {
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  GPT-VARIANTEN VERGLEICHSTEST - Aufmaßblatt OCR');
  console.log('═══════════════════════════════════════════════════════════════');
  ...
  for (const variant of VARIANTS) {
    ...
    if (result.success) {
      console.log(`  ✅ Erfolgreich in ${result.duration.toFixed(2)}s`);
      console.log(`  📊 Tokens: ${result.tokens.total} (davon ${result.tokens.reasoning} Reasoning)`);
  ...
  if (best) {
    console.log(`\n🏆 BESTE VARIANTE: ${best.variant} mit Score ${best.evaluation.score}/100`);
    console.log('\nDetails:');
